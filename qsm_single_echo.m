...
TE = 10*1e-3; % echo time for each GRE image, in second
simulated_TE = TE;       % you can set simulated_TE to some other values other than TE, the chosen TE (in second) used to simulate the phase image (in radian) from the averaged local fields (in Hz)
...
% in the single-echo case, iMagWtd is the same as the magnitude image
...
% the initial phase cannot be computed using a single echo....
iField_phase = zeros(size(iField));
for (i=1:size(iField,4))
    iField_phase(:,:,:,i) = angle(iField(:,:,:,i));
end
